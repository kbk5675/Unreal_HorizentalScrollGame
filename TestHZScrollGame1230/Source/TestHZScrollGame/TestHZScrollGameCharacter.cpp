...
{
	UE_LOG(LogTemp, Warning, TEXT("ATestHZScrollGameCharacter"));
...
void ATestHZScrollGameCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(ThisClass, RandNum);
}

void ATestHZScrollGameCharacter::BeginPlay()
{
	Super::BeginPlay();
	if (HasAuthority())
	{
		RandNum = FMath::Rand() % 10;
		NameText->SetText(FText::FromString(FString::FromInt(RandNum)));
	}
}
...
void ATestHZScrollGameCharacter::OnRep_SetNameText()
{
	NameText->SetText(FText::FromString(FString::FromInt(RandNum)));
}

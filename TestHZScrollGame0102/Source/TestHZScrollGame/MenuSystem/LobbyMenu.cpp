...
bool ULobbyMenu::Initialize()
{
	...
	GameInstance->ResetTPlayerNames();
...
void ULobbyMenu::PressedOnBtnFinish()
{
	...
	auto MyCharacter = Cast<ATestHZScrollGameCharacter>(Controller->GetPawn());

	if (MyCharacter->HasAuthority())
	{	// Server
		MyCharacter->ApplyName(PlayerName);
	}
	else
	{	// Client
		MyCharacter->ServerApplyName(PlayerName);
	}

	auto GameInstance = Cast<UTestHZScrollGameInstance>(GetGameInstance());
	if (!ensure(GameInstance != nullptr)) return;

	GameInstance->AddTPlayerName(PlayerName);

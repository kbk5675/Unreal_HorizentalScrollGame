...
void UTestHZScrollGameInstance::TravelStage1()
{
	if (GEngine)
	{
	...
	UWorld* World = GetWorld();
	if (!ensure(World != nullptr)) return;
	World->ServerTravel("/Game/Maps/Stage1?listen");
}
...
void UTestHZScrollGameInstance::LoadStage2()
{
	APlayerController* PlayerController = GetFirstLocalPlayerController();
	if (!ensure(PlayerController != nullptr)) return;

	PlayerController->ClientTravel("/Game/Maps/Lobby?listen", ETravelType::TRAVEL_Absolute);
}
